...
//
// SHARED-HARDWARE NOTE: this module is now instantiated exactly ONCE, inside
// keccak_shared_arbiter.  The added `idle` output lets the arbiter hand the
// core over to the KDF only when nothing is in flight.
//-----------------------------------------------------------------------------
module sha3_shake_core(
    ...
    output wire        idle         // high: no permute/absorb pending AND no
                                    //      undrained squeeze block (arbiter use)
);
    ...
    assign idle       = !permuting && !have_block && !ready_flag;
