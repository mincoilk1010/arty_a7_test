...
module Kyber_Server(
    ...
    // NEW: shared Keccak bus - pass-through to hash_core_Server instance
    // "hash" below. Threaded up to kyber_axi_wrapper / riscv_soc /
    // Kyber_System_Top where the arbiter actually lives.
    input               hash_grant,
    output              hash_keccak_init,
    output              hash_keccak_init_hard,
    output [1:0]        hash_keccak_mode,
    output              hash_keccak_wr_en,
    output [31:0]       hash_keccak_din,
    output              hash_keccak_wr_xor,
    output              hash_keccak_wr_last,
    output              hash_keccak_rd_en,
    output              hash_keccak_rd_extend,
    input               sh_busy,
    input               sh_idle,
    input  [31:0]       sh_dout,
    input               sh_dout_valid,
    input               sh_done,
    input               sh_done_extend,
    input  [5:0]        sh_dout_rate_words
);
...
// NEW: shared Keccak bus (pass-through to Kyber_Server ports above)
