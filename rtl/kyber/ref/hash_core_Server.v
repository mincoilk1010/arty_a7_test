`timescale 1ns / 1ps
//-----------------------------------------------------------------------------
// hash_core_Server.v - REBUILT around the SHARED sha3_shake_core
//                      (via keccak_shared_arbiter, client 0)
//
// Changes vs the pre-sharing version:
//   * The internal sha3_shake_core instance is REMOVED.  The sponge now
//     lives once, inside keccak_shared_arbiter, and is shared with the
//     fuzzy-extractor KDF (client 1).
//   * All sponge-facing signals are module ports.  Every strobe the glue
//     generates (init / wr_en / rd_en / rd_extend) is additionally gated
//     with hash_grant so the Kyber datapath can never disturb a KDF
//     transaction while the arbiter has handed the core over.
//   * keccak_ready is masked with hash_grant so a KDF 'done' pulse can
//     never be mistaken for a Kyber block boundary by the parent FSM.
//
// Everything else (IFIFO arbitration, squeeze counters, ofifo routing,
// decode, tag capture) is unchanged.
//-----------------------------------------------------------------------------
...
module hash_core_Server(
    ...
    // --------------------------------------------------------------
    // Shared Keccak hardware (arbiter client 0)
    // --------------------------------------------------------------
    input  wire        hash_grant,            // low while KDF owns the core
    output wire        hash_keccak_init,      // 1-clk start-of-message pulse
    output wire        hash_keccak_init_hard, // hard message boundary
    output wire [1:0]  hash_keccak_mode,
    output wire        hash_keccak_wr_en,     // accept hash_keccak_din
    output wire [31:0] hash_keccak_din,
    output wire        hash_keccak_wr_xor,
    output wire        hash_keccak_wr_last,
    output wire        hash_keccak_rd_en,     // consume sh_dout
    output wire        hash_keccak_rd_extend,
    input  wire        sh_busy,
    input  wire        sh_idle,               // (unused here; arbiter use)
    input  wire [31:0] sh_dout,
    input  wire        sh_dout_valid,
    input  wire        sh_done,
    input  wire        sh_done_extend,
    input  wire [5:0]  sh_dout_rate_words
);
    ...
    // While the KDF owns the shared core (hash_grant low) the FIFO is frozen
    // so no word can leak into the KDF transaction.
    assign ififo_req = ~(ififo_empty | core_busy | extend) & hash_grant;
    ...
    // ------------------------------------------------------------------
    // Sponge glue (core itself lives in keccak_shared_arbiter)
    // ------------------------------------------------------------------
    wire        sponge_init;
    reg         init_d;
    ...
    // Alias the broadcast core status wires.
    assign core_busy          = sh_busy;
    assign sponge_dout        = sh_dout;
    assign sponge_valid       = sh_dout_valid;
    assign sponge_done        = sh_done;
    assign sponge_done_extend = sh_done_extend;
    assign sponge_output_rate = sh_dout_rate_words;
    ...
    wire [1:0] core_mode = ififo_mode;   // latched by core at init

    // Drive the shared core.  Every strobe is gated with hash_grant.
    assign hash_keccak_init       = sponge_init & hash_grant;
    assign hash_keccak_init_hard  = keccak_init_hard & hash_grant;
    assign hash_keccak_mode       = core_mode;
    assign hash_keccak_wr_en      = core_word_accept;
    assign hash_keccak_din        = core_din;
    assign hash_keccak_wr_xor     = absorb_w;
    assign hash_keccak_wr_last    = core_last;
    assign hash_keccak_rd_en      = sponge_rd_en;        // pre-gated below
    assign hash_keccak_rd_extend  = extend & hash_grant;
    ...
    assign core_word_accept = word_pend && !core_busy && !extend && hash_grant;
...
wire sponge_hold = sponge_hold_set & ~sponge_done;
    ...
    // Mask with hash_grant: a KDF 'done' pulse must never be seen as a
    // Kyber block boundary by the parent FSM.
    assign keccak_ready   = sponge_done & hash_grant;
    ...
    always @(posedge clk) begin
        ...
        else if (squeeze_init || (sponge_done && hash_grant) ||
                 (~extend & extend_r1))
            squeeze_ctr <= 6'h0;
